...
def dense_layer(units, activation=None, use_bias=False, name=None):
    ...
    fn = tf.keras.layers.Dense(
        units, activation=convert_activation_string(activation), use_bias=use_bias, name=name,
        kernel_regularizer=tf.keras.regularizers.L2(l2=_L2_SCALE))
    return fn
