...
class PFCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    """
    ...
    def transition_model(self, particle_states, odometry):
        ...
        delta_x += tf.random.normal(delta_x.get_shape(), mean=0.0, stddev=1.0) * translation_std
        delta_y += tf.random.normal(delta_y.get_shape(), mean=0.0, stddev=1.0) * translation_std
    ...
    def joint_vector_features(joint_vector):
        x = dense_layer(1, activation=None, use_bias=True, name='fc1')(joint_vector)
        return x
...
class PFNet(object):
    ...
    def build_train_op(self):
        ...
        print(f"IN BUILD TRAIN ITERATION")
        # make sure this is only called once
        # assert self.train_op is None and self.learning_rate_op is None
        ...
        with tf.device("/cpu:0"):
            # self.global_step_op = tf.compat.v1.get_variable(
            #     initializer=tf.compat.v1.constant_initializer(0.0), shape=(), trainable=False, name='global_step',)
            ...
            # self.learning_rate_op = tf.keras.optimizers.schedules.ExponentialDecay(
            #     self.params.learningrate, decay_steps=1, decay_rate=self.params.decayrate,
            #     staircase=True, name="learning_rate")
    ...
    def build_rnn(self, global_maps, init_particle_states, observations, odometries, is_first_step):
        ...
        """
        print(f"IN BUILD RUN ITERATION, {len(self.hidden_states)}")
        ...
        # self.hidden_states = [
        #     tf.compat.v1.get_variable("particle_states", shape=init_particle_states.get_shape(),
        #                     dtype=init_particle_states.dtype, initializer=tf.constant_initializer(0), trainable=False),
        #     tf.compat.v1.get_variable("particle_weights", shape=init_particle_weights.get_shape(),
        #                     dtype=init_particle_weights.dtype, initializer=tf.constant_initializer(0), trainable=False),
        #     ]
        self.hidden_states = [
            tf.Variable(tf.constant_initializer(0)(shape=init_particle_states.get_shape(), dtype=init_particle_states.dtype), trainable = False, name = "particle_states"),
        ...
        # outputs, state = tf.compat.v1.nn.dynamic_rnn(cell=cell_func,
        ...
        (particle_states, particle_weights), states, weights = rnn(inputs = (observations, odometries), initial_state = state)
