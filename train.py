...
def run_training(params):
    ...
    with tf.Graph().as_default():
        ...
        saver = tf.compat.v1.train.Saver(var_list=tf.compat.v1.trainable_variables(), max_to_keep=3)
...
def loook_data(params):
    """ Run training with the parsed arguments """

    train_data, num_train_samples = get_dataflow(params.trainfiles, params, is_training=True)
    print("len train data", len(train_data), "nts", num_train_samples)
    for eidx, train_sub in enumerate(train_data):
        print(f"len train sub {eidx}", train_sub.shape)
